...
const AppCard: React.FC<AppCardProps> = ({ app, index, isLoggedIn }) => {
      ...
      if (isExternal && isLoggedIn) {
        ...
        switch(true){
          ...
          case app.route?.includes('todo'):
            ...
            break
          case app.route?.includes('aimage'):
  ...
  return (
  <motion.div 
    onClick={isComingSoon ? undefined : handleClick} // ← disable click
    layout
    initial={{ opacity: 0, scale: 0.8, y: 20 }}
    animate={{ opacity: 1, scale: 1, y: 0 }}
    exit={{ opacity: 0, scale: 0.8, y: 20, transition: { duration: 0.08 } }}
    ...
    className={`group flex flex-col items-center gap-3 sm:gap-4 w-full pb-4 pt-2 
      ${isComingSoon ? 'cursor-not-allowed opacity-40 grayscale' : 'cursor-pointer'}`} // ← grey out
  >
    <div className="relative">
      <motion.div 
        whileHover={isComingSoon ? {} : {  // ← disable hover animation
          scale: 1.05, 
          y: -8,
          transition: { type: 'spring', stiffness: 400, damping: 12 }
        }}

...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checksesionid = async () => {
      ...
      api
        .get(`https://sso.snabbb.com/api/redirect?sid=${session_id}`)
        .then((res) => res.data)
        .then((data) => {
  ...
  // Listen for logout messages from mini apps
  useEffect(() => {
  ...
  // Fallback: re-check auth when this tab becomes active again
  useEffect(() => {
    const onFocus = () => {
      verifySession();
    };

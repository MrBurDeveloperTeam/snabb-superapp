        ...
        <section className="pt-12 sm:pt-20 pb-12 text-center flex flex-col items-center border-b border-slate-100/50 mb-8">
          ...
          >
            ...
            <p className="text-slate-600 text-lg md:text-xl font-light max-w-3xl mx-auto leading-relaxed mb-12">
              {isLoggedIn
                ? `Welcome back, ${user?.fullName?.split(' ')[0] || 'User'}! Discover our premium collection.`
      ...
      <footer className="max-w-7xl mx-auto px-6 mt-12 pb-12">
        <div className="py-12 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-8">
            <NavFooterButton to="/privacy">Privacy</NavFooterButton>
            <NavFooterButton to="/terms">Terms</NavFooterButton>
          </div>
        </div>
      </footer>
    </>
...
const NavFooterButton: React.FC<{ to: string; children: React.ReactNode }> = ({ to, children }) => {
  ...
  return (
    <button
      ...
      className={`transition-colors text-xs font-black uppercase tracking-widest ${
        location.pathname === to ? 'text-blue-600' : 'text-slate-400 hover:text-blue-600'
      }`}
    >
      {children}
    </button>
...
const AuthRoutePage: React.FC<AuthRoutePageProps> = ({
  authMode,
  isLoggedIn,
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    run();
  }, [location.pathname, verifySession, navigate]);

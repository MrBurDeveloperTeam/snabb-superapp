...
const TutorialWatchPage: React.FC<TutorialWatchPageProps> = ({ videoId, onNavigate }) => {
  ...
  if (!video) {
    return (
      <div className="tutorial-page flex min-h-[60vh] flex-col items-center justify-center px-6 text-center">
        ...
        <article>
          ...
          <div className="mt-6">
            ...
            <div className="mt-7 flex items-center gap-3 border-t border-slate-200 pt-6">
              <span className="tutorial-mini-app-icon flex h-10 w-10 items-center justify-center rounded-xl bg-tiffany-100 text-tiffany-600"><BookOpenText size={20} /></span>
          ...
          <div className="space-y-5">
            {upNext.map((item) => (
              <button key={item.id} type="button" onClick={() => onNavigate(`/tutorial-video/${item.id}`)} className="group flex w-full gap-3 text-left">
                ...
                <span className="min-w-0 py-1"><span className={`tutorial-category inline-block rounded border px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wider ${getTutorialCategoryStyle(item.category)}`}>{item.category}</span><span className="mt-1 block text-sm font-extrabold leading-5 text-slate-800 group-hover:text-tiffany-700">{item.title}</span></span>
              </button>
            ))}
          </div>
          <button type="button" onClick={() => onNavigate('/tutorial-video')} className="tutorial-back-button mt-8 flex w-full items-center justify-center gap-2 rounded-xl border border-tiffany-200 bg-tiffany-100/70 px-5 py-3.5 text-sm font-extrabold text-tiffany-700 hover:bg-tiffany-100"><ArrowLeft size={16} /> Back to Tutorial Library</button>
        </aside>
      </main>
    </div>
  );

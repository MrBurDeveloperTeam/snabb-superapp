...
export default function TicketingDashboard({ isAdmin, ticketId, userName, onNavigate }: Props) {
  ...
  return <main className="ticketing-theme min-h-[calc(100vh-88px)] bg-slate-50 text-slate-900">
    ...
    {selected&&<div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10"><button type="button" onClick={()=>onNavigate(dashboardPath)} className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-blue-600"><ArrowLeft size={16}/> Back to tickets</button><article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="mb-6 flex flex-col items-start justify-between gap-4 border-b border-slate-100 pb-6 sm:flex-row"><div><span className="text-xs font-extrabold uppercase tracking-wider text-blue-600">{formatTicket(selected.ticket_no)}</span><h1 className="mt-2 text-2xl font-black sm:text-3xl">{selected.subject}</h1><p className="mt-2 text-xs text-slate-400">Created {formatDate(selected.created_at)} · {priorityLabel[selected.priority]} priority</p></div><span className={`inline-flex rounded-full border px-3 py-1.5 text-xs font-extrabold ${statusMeta[selected.status].color}`}>{statusMeta[selected.status].label}</span></div>{error&&<div className="mb-6 flex items-center gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700"><AlertCircle size={18}/><span className="flex-1">{error}</span><button type="button" onClick={()=>setError('')} aria-label="Dismiss error"><X size={16}/></button></div>}{isAdmin&&<div className="mb-5"><p className="mb-2 text-xs font-bold uppercase text-slate-400">Change status</p><div className="flex flex-wrap gap-2">{Object.keys(statusMeta).map((code)=><button disabled={saving} onClick={()=>void changeStatus(code as TicketStatusCode)} key={code} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${selected.status===code?statusMeta[code as TicketStatusCode].color:'border-slate-200 text-slate-500'}`}>{statusMeta[code as TicketStatusCode].label}</button>)}</div></div>}<div className="rounded-2xl bg-slate-50 p-5"><p className="whitespace-pre-wrap text-sm leading-7 text-slate-600">{selected.description}</p></div>

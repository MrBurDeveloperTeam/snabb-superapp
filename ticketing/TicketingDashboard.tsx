    ...
    {selected&&<div className="mx-auto flex w-full max-w-[1440px] flex-col px-4 py-6 sm:px-6 lg:h-[calc(100vh-88px)] lg:overflow-hidden lg:px-10 lg:py-7">
      ...
      <article className="grid min-h-0 flex-1 gap-5 lg:grid-cols-[minmax(0,0.86fr)_minmax(440px,1.14fr)]">
        ...
        <section className="flex min-h-0 flex-col overflow-hidden rounded-[28px] border border-white/80 bg-white/95 shadow-[0_20px_60px_-32px_rgba(15,23,42,0.35)] ring-1 ring-slate-200/70">
          ...
          {!['done','expired'].includes(selected.status)&&<form onSubmit={submitReply} className="shrink-0 border-t border-slate-100 bg-white p-5 sm:px-7"><div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-2 transition focus-within:border-tiffany-300 focus-within:bg-white focus-within:ring-4 focus-within:ring-tiffany-50"><textarea maxLength={20000} rows={3} value={reply} onChange={(e)=>setReply(e.target.value)} placeholder={isAdmin?'Write a reply to the customer…':'Add a reply…'} className="w-full resize-none bg-transparent px-3 py-2 text-sm leading-6 outline-none placeholder:text-slate-400"/><div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200/70 px-2 pt-2"><div className="flex items-center gap-3"><input ref={replyFileRef} type="file" multiple className="hidden" onChange={(e)=>setReplyFiles(Array.from(e.target.files||[]).slice(0,5))}/><button type="button" onClick={()=>replyFileRef.current?.click()} className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-xs font-bold text-slate-500 transition hover:bg-white hover:text-blue-600"><FileUp size={15}/>{replyFiles.length?`${replyFiles.length} file(s)`:'Attach files'}</button>{isAdmin&&<label className="flex items-center gap-2 text-xs font-bold text-amber-700"><input type="checkbox" checked={internalNote} onChange={(e)=>setInternalNote(e.target.checked)} className="accent-amber-600"/> Internal note</label>}</div><button disabled={saving||(!reply.trim()&&!replyFiles.length)} className="flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-bold text-white shadow-lg shadow-tiffany-600/20 transition hover:-translate-y-0.5 hover:shadow-xl disabled:translate-y-0 disabled:opacity-50"><Send size={15}/> Send</button></div></div></form>}

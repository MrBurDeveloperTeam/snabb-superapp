...
export default function TicketingDashboard({ isAdmin, ticketId, userName, onNavigate }: Props) {
  ...
  return <main className="ticketing-theme min-h-[calc(100vh-88px)] bg-slate-50 text-slate-900">
    ...
    {selected&&<div className="mx-auto flex w-full max-w-7xl flex-col px-4 py-6 sm:px-6 lg:h-[calc(100vh-88px)] lg:overflow-hidden lg:px-8 lg:py-7">
      <button type="button" onClick={()=>onNavigate(dashboardPath)} className="mb-4 inline-flex shrink-0 items-center gap-2 self-start text-sm font-bold text-slate-500 hover:text-blue-600"><ArrowLeft size={16}/> Back to tickets</button>
      <article className="grid min-h-0 flex-1 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[minmax(0,0.9fr)_minmax(420px,1.1fr)]">
        <section className="min-h-0 p-6 sm:p-8 lg:overflow-y-auto">
          <div className="mb-6 flex flex-col items-start justify-between gap-4 border-b border-slate-100 pb-6 sm:flex-row"><div><span className="text-xs font-extrabold uppercase tracking-wider text-blue-600">{formatTicket(selected.ticket_no)}</span><h1 className="mt-2 text-2xl font-black sm:text-3xl">{selected.subject}</h1><p className="mt-2 text-xs text-slate-400">Created {formatDate(selected.created_at)} · {priorityLabel[selected.priority]} priority</p></div><span className={`inline-flex rounded-full border px-3 py-1.5 text-xs font-extrabold ${statusMeta[selected.status].color}`}>{statusMeta[selected.status].label}</span></div>
          ...
          {isAdmin&&<div className="mb-5"><p className="mb-2 text-xs font-bold uppercase text-slate-400">Change status</p><div className="flex flex-wrap gap-2">{Object.keys(statusMeta).map((code)=><button disabled={saving} onClick={()=>void changeStatus(code as TicketStatusCode)} key={code} className={`rounded-lg border px-3 py-1.5 text-xs font-bold ${selected.status===code?statusMeta[code as TicketStatusCode].color:'border-slate-200 text-slate-500'}`}>{statusMeta[code as TicketStatusCode].label}</button>)}</div></div>}
          <div className="rounded-2xl bg-slate-50 p-5"><p className="whitespace-pre-wrap text-sm leading-7 text-slate-600">{selected.description}</p></div>
        </section>

        <section className="flex min-h-0 flex-col border-t border-slate-200 bg-slate-50/50 lg:border-l lg:border-t-0">
          <div className="shrink-0 border-b border-slate-200 bg-white px-6 py-5 sm:px-7"><h3 className="flex items-center gap-2 text-sm font-black uppercase tracking-wider text-slate-500"><MessageSquare size={16}/> Conversation</h3><p className="mt-1 text-xs font-medium text-slate-400">{messages.length} {messages.length===1?'reply':'replies'}</p></div>
          <div className="min-h-[280px] flex-1 overflow-y-auto overscroll-contain px-6 py-5 sm:px-7 lg:min-h-0" tabIndex={0} aria-label="Ticket conversation">
            {detailLoading?<div className="flex h-full items-center justify-center"><LoaderCircle className="animate-spin text-blue-600"/></div>:<div className="space-y-3">{messages.length===0&&<p className="rounded-xl border border-dashed border-slate-200 bg-white p-5 text-center text-sm text-slate-400">No replies yet.</p>}{messages.map((message)=><article key={message.id} className={`rounded-2xl border p-4 ${message.is_internal?'border-amber-200 bg-amber-50':'border-slate-200 bg-white'}`}><div className="mb-2 flex justify-between gap-3"><b className="text-sm">{message.author_name||message.author_email||'Snabbb User'}{message.is_internal&&<span className="ml-2 text-[10px] text-amber-700">INTERNAL NOTE</span>}</b><time className="text-[10px] text-slate-400">{formatDate(message.created_at)}</time></div><p className="whitespace-pre-wrap text-sm leading-6 text-slate-600">{message.body}</p>{attachments.filter((a)=>a.message_id===message.id).map((a)=><a key={a.id} href={a.signed_url} target="_blank" rel="noreferrer" className="mt-3 flex items-center gap-2 text-xs font-bold text-blue-600"><Download size={14}/>{a.file_name}</a>)}</article>)}{attachments.filter((a)=>!a.message_id).map((a)=><a key={a.id} href={a.signed_url} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-xl bg-blue-50 p-3 text-xs font-bold text-blue-600"><Download size={14}/>{a.file_name}</a>)}</div>}
          </div>
          {!['done','expired'].includes(selected.status)&&<form onSubmit={submitReply} className="shrink-0 border-t border-slate-200 bg-white p-5 sm:px-7"><textarea required maxLength={20000} rows={3} value={reply} onChange={(e)=>setReply(e.target.value)} placeholder={isAdmin?'Write a reply to the customer…':'Add a reply…'} className="w-full resize-none rounded-xl border border-slate-200 p-4 text-sm outline-none focus:border-blue-400"/><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><input ref={replyFileRef} type="file" multiple className="hidden" onChange={(e)=>setReplyFiles(Array.from(e.target.files||[]).slice(0,5))}/><button type="button" onClick={()=>replyFileRef.current?.click()} className="flex items-center gap-2 text-xs font-bold text-slate-500"><FileUp size={16}/>{replyFiles.length?`${replyFiles.length} file(s)`:'Attach files'}</button>{isAdmin&&<label className="flex items-center gap-2 text-xs font-bold text-amber-700"><input type="checkbox" checked={internalNote} onChange={(e)=>setInternalNote(e.target.checked)}/> Internal note</label>}</div><button disabled={saving||!reply.trim()} className="flex h-10 items-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-bold text-white disabled:opacity-50"><Send size={15}/> Send</button></div></form>}
        </section>
      </article>
    </div>}
  </main>;

...
export interface PaymentProvider {
  ...
  inline: boolean;
  /** Stripe only. Odoo's own /payment/init response (processing_values)
   *  carries client_secret but never a publishable key — Odoo's hosted
   *  /shop/payment template reads that straight off the provider record
   *  server-side. This mirrors that: the key rides along on the provider
   *  list instead, since Stripe.js needs it before it can even consume
   *  payment/init's client_secret. */
